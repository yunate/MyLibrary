...
std::string WinRegistryKey::getString(const std::string& name)
{
    ...
    if (RegQueryValueExA(m_hKey, name.c_str(), NULL, &type, NULL, &size) != ERROR_SUCCESS || (type != REG_SZ && type != REG_EXPAND_SZ && type != REG_LINK))
    {
        return std::string();
    }
    ...
    if (size > 0)
    {
        ...
        size += 1;
        RegQueryValueExA(m_hKey, name.c_str(), NULL, NULL, (BYTE*) buffer.begin(), &size);
        buffer[size] = 0;
...
std::vector<char> WinRegistryKey::getBinary( const std::string& name )
{
    ...
    if (RegQueryValueExA(m_hKey, name.c_str(), NULL, &type, NULL, &size) != ERROR_SUCCESS || type != REG_BINARY)
    {
        return std::vector<char>();
    }
    ...
    {
        result.resize(size);
        RegQueryValueExA(m_hKey, name.c_str(), NULL, NULL, (BYTE*) &result[0], &size);
    }
...
bool WinRegistryKey::getInt( const std::string& name, int& value )
{
    ...
    if (RegQueryValueExA(m_hKey, name.c_str(), NULL, &type, (BYTE*) &data, &size) != ERROR_SUCCESS || (type != REG_DWORD && type != REG_DWORD_BIG_ENDIAN))
    {
        return false;
    }
...
bool WinRegistryKey::getInt64( const std::string& name, WinRegistryKey::Int64& value )
{
    ...
    if (RegQueryValueExA(m_hKey, name.c_str(), NULL, &type, (BYTE*) &data, &size) != ERROR_SUCCESS || type != REG_QWORD)
    {
        return false;
    }
...
WinRegistryKey::Type WinRegistryKey::type(const std::string& name)
{
    ...
    if (RegQueryValueExA(m_hKey, name.c_str(), NULL, &type, NULL, &size) != ERROR_SUCCESS)
    {
        return REGT_ERROR;
    }
...
bool WinRegistryKey::open()
{
    if (!m_hKey)
    {
        ...
        {
            if (RegCreateKeyExA(m_hRootKey, m_subKey.c_str(), 0, NULL, REG_OPTION_NON_VOLATILE, KEY_READ | KEY_WRITE | m_extraSam, NULL, &m_hKey, NULL) != ERROR_SUCCESS)
            {
                return false;
            }
        }
...
void WinRegistryKey::subKeys(WinRegistryKey::Keys& keys)
{
    ...
    if (RegQueryInfoKey(m_hKey, NULL, NULL, NULL, &subKeyCount, NULL, NULL, &valueCount, NULL, NULL, NULL, NULL) != ERROR_SUCCESS)
    {
        return;
    }
    ...
    {
        if (RegEnumKeyExA(m_hKey, i, buf, &bufSize, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
        {
            std::string name(buf);
            keys.push_back(name);
        }
...
void WinRegistryKey::values(WinRegistryKey::Values& vals)
{
    ...
    if (RegQueryInfoKey(m_hKey, NULL, NULL, NULL, NULL, NULL, NULL, &valueCount, NULL, NULL, NULL, NULL) != ERROR_SUCCESS)
    {
        return;
    }
    ...
    {
        if (RegEnumValueA(m_hKey, i, buf, &bufSize, NULL, NULL, NULL, NULL) == ERROR_SUCCESS)
        {
            std::string name(buf);
            vals.push_back(name);
        }
