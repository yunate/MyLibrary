...
void PipeManager::StopLoop()
{
    ...
    if (m_spLoopThread != nullptr && m_spLoopThread->IsRunning())
    {
        ...
        m_spLoopThread->TimeWait(PIPE_DEFAULT_TIMEOUT);
...
bool PipeManager::CreatePipe(const std::wstring& name, bool isServer, bool sync/* = false*/)
{
    ...
    if (sync)
    {
        ...
        std::shared_ptr<SimpleEvent> spWaitEvent(new(std::nothrow)SimpleEvent());
        ...
        ipc::MessageLoopForIPC::GetInstace()->PostTask(
            ipc::Bind(&PipeManager::OnCreate,
                      ...
                      name,
                      isServer,
                      std::shared_ptr<SimpleEvent>(spWaitEvent),
                      std::shared_ptr<bool>(spResult)));

        spWaitEvent->TimeWait(PIPE_DEFAULT_TIMEOUT);
        result = *spResult;
    }
    else
    {
...
void PipeManager::OnCreate(const std::wstring& name,
                             bool isServer,
                             const std::weak_ptr<SimpleEvent>& spEvent,
...
void PipeManager::Close(const std::wstring& name, bool sync/* = false*/)
{
    ...
    if (sync)
    {
        ...
        // 放到消息进程中，否则内部的pipe初始化会有异步问题
        ipc::MessageLoopForIPC::GetInstace()->PostTask(
            ipc::Bind(&PipeManager::OnClose, this, pipe, std::weak_ptr<SimpleEvent>(spWaitEvent)));
    ...
    else
    {
        // 放到消息进程中，否则内部的pipe初始化会有异步问题
        ipc::MessageLoopForIPC::GetInstace()->PostTask(
            ipc::Bind(&PipeManager::OnClose, this, pipe, std::weak_ptr<SimpleEvent>()));
    }
}

void PipeManager::OnClose(SPSyncPipe spPipe, const std::weak_ptr<SimpleEvent>& spEvent)
{
    if (spPipe != nullptr)
    {
        spPipe->Close();
    ...
    if (sync)
    {
        ...
        ipc::MessageLoopForIPC::GetInstace()->PostTask(
            ipc::Bind(&PipeManager::OnCloseAndReCreate,
                      ...
                      pipe,
                      isServer,
                      std::weak_ptr<SimpleEvent>(spWaitEvent),
