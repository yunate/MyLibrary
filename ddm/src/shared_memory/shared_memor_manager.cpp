#ifdef _WINDOWS
#include "shared_memor_manager.h"

BEG_NSP_DDM
shared_memor_manager::shared_memor_manager() {}
...
sp_shared_memory shared_memor_manager::create(const ddstr name, size_t size)
{
...
sp_shared_memory shared_memor_manager::open(const ddstr name)
{
...
void NSP_DDM::shared_memor_manager::SetTmpDir(const ddstr& tmpDir)
{
    m_tmpDir = tmpDir;
}

HANDLE shared_memor_manager::find(const ddstr name)
{
